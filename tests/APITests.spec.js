...
// Global setup for the API context and token. These are needed
let apiContext;
let authToken;
...
test('Verify all order details from /orders API', async () => {
    ...
    // Verify order API returns successful 200 response
    expect(ordersResponse.status()).toBe(200);
    
    const ordersData = await ordersResponse.json();
    console.log('Orders Response:', ordersData);
  
    // Verify each captured order details
...
test('Verify number of orders created before the current date', async () => {
    ...
    expect(numberOfOrdersBeforeToday).toBeGreaterThan(0); // Checks that the outcome is greater than 1
});
